...
# Create ilens namespace
kubectl create namespace ilens

# Uninstall the Helm releases if they exist
helm uninstall -n ilens "app-services" > /dev/null 2>&1 || true
helm uninstall -n ilens "app-core" > /dev/null 2>&1 || true
...
if helm install -n ilens app-services ./app \
...
while [ -z "$mediamtx_service_ip" ]; do
    mediamtx_service_ip=$(kubectl get node ubuntu-10-11 -o jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}' | awk -F " " '{print $1}')
    # mediamtx_service_ip=$(kubectl get service mediamtx-service -n ilens -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
# If the installation fails, print an error message and exit the script.
if helm install -n ilens app-core ./app \
